...
class MyColorAnalyzer(GreyYUVAnalysis):
    def __init__(self, camera):
        ...
        time.sleep(2)
        self.output_stream = cs.putVideo(
            "Processed", self.camera.resolution.width, self.camera.resolution.height
        )
        self.camera_params = [
            357.1,
            357.1,
            self.camera.resolution.width / 2,
            self.camera.resolution.height / 2,
        ]
    ...
    def analyze(self, a):
        ...
        draw_result(output_img, self.camera_params, self.tag_size, result)
        ...
        for r in result:
            ...
            if r.pose_R is not None:
                for i in range(3):
                    ap_pose_R_x_str = (
                        ap_pose_R_x_str + str(round(r.pose_R[0][i], 4)) + " "
                    )
                    ap_pose_R_y_str = (
                        ap_pose_R_y_str + str(round(r.pose_R[1][i], 4)) + " "
                    )
                    ap_pose_R_z_str = (
                        ap_pose_R_z_str + str(round(r.pose_R[2][i], 4)) + " "
                    )
...
def main():
    ...
    width = 576
    height = 320
    camera = PiCamera(resolution=(width, height), framerate=28, sensor_mode=5)
